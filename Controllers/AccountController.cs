        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    location.flatNumber = model.FlatNumber;
                    location.roomNumber = Convert.ToInt32(model.RoomNumber);
                    location.door = model.Door;
                    location.floor = Convert.ToInt32(model.Floor);

                    db.Locations.Add(location);
                    db.SaveChanges();

                    person.firstName = model.FirstName;
                    person.lastName = model.SecondName;
                    person.emailAddress = model.Email;
                    person.isAdmin = false;
                    person.isMaintenance = false;
